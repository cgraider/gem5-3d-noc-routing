...
# Neural Network
# ---------------------------------------------------------------------------
# [فارسی] DQN: شبکهٔ عصبیِ تخمین‌زنندهٔ مقدارِ Q. ورودی = بردارِ حالت، خروجی = ۶ عدد
# (ارزشِ هر جهت). لایهٔ اولِ compress هر اندازهٔ ورودی را به ۶۴ تبدیل می‌کند تا بقیهٔ
# شبکه مستقل از اندازهٔ شبکه ثابت بماند.
class DQN(nn.Module):
    """
    ...
    # [فارسی] constructor: لایه‌های شبکه را تعریف می‌کند.
    ...
    # [فارسی] forward: عبورِ روبه‌جلو؛ ورودی را از لایه‌ها رد می‌کند و Qِ خام برمی‌گرداند.
...
# Experience Replay
# ---------------------------------------------------------------------------
# [فارسی] ReplayBuffer: بافرِ تجربه؛ چهارتایی‌های (حالت، اکشن، پاداش، حالتِ بعدی، done)
# را نگه می‌دارد. deque با maxlen خودش قدیمی‌ها را حذف می‌کند (صفِ حلقوی).
class ReplayBuffer:
    def __init__(self, capacity: int = REPLAY_BUFFER_SIZE):
        self.buf = deque(maxlen=capacity)

    # [فارسی] push: یک تجربه به بافر اضافه می‌کند.
    def push(self, state, action, reward, next_state, done):
        self.buf.append((state, action, reward, next_state, done))

    # [فارسی] sample: یک دستهٔ تصادفی برمی‌دارد و به تنسورهای PyTorch تبدیل می‌کند.
...
# Agent
# ---------------------------------------------------------------------------
# [فارسی] ProposedAgent: خودِ عاملِ یادگیرنده. دو شبکه دارد: q_net (اصلی) و
# target_net (هدف، یک کپیِ کندتر برای پایداری). بهینه‌سازِ Adam و بافرِ تجربه هم اینجاست.
class ProposedAgent:
    # [فارسی] constructor: دو شبکه، بهینه‌ساز، بافر و پارامترِ اکتشاف (epsilon) را می‌سازد.
    def __init__(self, state_size: int, action_size: int = 6,
    ...
    # ------------------------------------------------------------------
    # [فارسی] select_action: سیاستِ epsilon-greedy با ماسک. با احتمالِ epsilon یک جهتِ
    # مجازِ تصادفی (اکتشاف)؛ وگرنه جهتی با بیشترین Q (بهره‌برداری). جهت‌های نامجاز با
    # -inf ماسک می‌شوند تا هرگز انتخاب نشوند.
    ...
    # ------------------------------------------------------------------
    # [فارسی] remember: یک تجربه را به بافر می‌سپارد.
    def remember(self, state, action, reward, next_state, done):
        self.memory.push(state, action, reward, next_state, done)

    # ------------------------------------------------------------------
    # [فارسی] train: یک گامِ یادگیری. هدف را حساب می‌کند، خطای Huber می‌گیرد، گرادیان
    # می‌زند، epsilon را کم می‌کند و هر چند گام شبکهٔ هدف را هم‌گام می‌کند.
    def train(self) -> float:
        ...
        # [فارسی] Qِ فعلی برای اکشن‌هایی که واقعاً انتخاب شده‌اند.
        ...
        # [فارسی] خطای Huber (به‌جای MSE در DeepNR): در برابرِ پاداش‌های پرت مقاوم‌تر است.
    ...
    # ------------------------------------------------------------------
    # [فارسی] save / load: ذخیره و بازیابیِ وزن‌ها، بهینه‌ساز، epsilon و شمارنده‌ها در فایلِ .pth
...
# ZMQ Server
# ---------------------------------------------------------------------------
# [فارسی] ProposedServer: سرورِ ZMQ که روی پورتِ 5556 منتظرِ gem5 می‌ماند، حالت را
# می‌گیرد، اکشن برمی‌گرداند و عامل را آموزش می‌دهد.
class ProposedServer:
    DIR_NAMES = ["North", "East", "South", "West", "Up", "Down"]

    # [فارسی] constructor: سوکتِ ZMQ از نوعِ REP (پاسخ‌دهنده) را می‌سازد و به پورت می‌بندد.
    ...
    # [فارسی] run: حلقهٔ اصلیِ سرور. در هر تکرار: حالت را از gem5 می‌گیرد، تجربهٔ گامِ
    # قبل را ذخیره می‌کند، اکشن انتخاب و ارسال می‌کند، و در صورتِ لزوم آموزش می‌دهد.
    def run(self):
        last_state  = None
        last_action = None
        step        = 0
        ...
        while self.running:
            ...
            # [فارسی] حالا که پاداشِ گامِ قبل رسیده، آن تجربه را کامل و ذخیره می‌کنیم.
...
# Main
# ---------------------------------------------------------------------------
# [فارسی] main: آرگومان‌ها را می‌خواند، اندازهٔ حالت را از ابعادِ شبکه حساب می‌کند
# (2*num_routers + 28)، عامل و سرور را می‌سازد و سرور را اجرا می‌کند.
def main():
    ...
    # [فارسی] اندازهٔ حالت باید دقیقاً با چیزی که gem5 می‌فرستد یکی باشد، وگرنه عامل
    # سرِ هر تصمیم هشدار می‌دهد.
