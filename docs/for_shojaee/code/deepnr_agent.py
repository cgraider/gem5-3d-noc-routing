...
# [فارسی] ===========================================================================
# عاملِ پایتونیِ DeepNR3D (الگوریتم ۲). یک سرورِ ZMQ که روی پورتِ 5555 منتظرِ gem5 می‌ماند.
# ساختار: DQN (شبکهٔ عصبی) + ReplayBuffer (بافرِ تجربه) + DeepNR_Agent (منطقِ یادگیری)
# + DeepNR_Server (حلقهٔ ارتباط با gem5). بخش‌های داشبورد/گزارش اختیاری‌اند و می‌توان
# نادیده‌شان گرفت. هستهٔ ماجرا همان چهار جزءِ بالاست.
# ===========================================================================
import argparse
import glob
import os
...
class DQN(nn.Module):
    ...
    # [فارسی] constructor: پنج لایهٔ شبکه را تعریف می‌کند. لایهٔ fc_compress هر اندازهٔ ورودی
    # را به ۶۴ تبدیل می‌کند تا شبکه با اندازه‌های مختلفِ مش کار کند.
    ...
    # [فارسی] forward: عبورِ روبه‌جلو؛ بردارِ حالت را به ۶ مقدارِ Q (ارزشِ هر جهت) تبدیل می‌کند.
    def forward(self, x):
...
class ReplayBuffer:
    ...
    # [فارسی] بافرِ تجربه با ظرفیتِ پیش‌فرضِ ۲۰۰ (طبقِ مقاله؛ اسکریپتِ آموزش آن را بزرگ‌تر می‌کند).
    def __init__(self, capacity=200):
        self.capacity = capacity
        self.buffer = deque(maxlen=capacity)

    # [فارسی] push: یک تجربهٔ (حالت، اکشن، پاداش، حالتِ بعدی، done) را اضافه می‌کند.
    def push(self, state, action, reward, next_state, done):
        """Add an experience to the buffer."""
        self.buffer.append((state, action, reward, next_state, done))

    # [فارسی] sample: یک دستهٔ تصادفی برمی‌دارد و به تنسور تبدیل می‌کند (شکستنِ همبستگیِ
    # تجربه‌های پشتِ‌سرِهم).
...
class DeepNR_Agent:
    ...
    # [فارسی] constructor: دو شبکهٔ q_network و target_network، بهینه‌سازِ Adam و بافرِ تجربه
    # را می‌سازد. آرگومان‌ها همان ابرپارامترهای یادگیری‌اند (نرخِ یادگیری، گاما، epsilon...).
    def __init__(
        self,
    ...
    # [فارسی] update_target_network: وزن‌های شبکهٔ اصلی را در شبکهٔ هدف کپی می‌کند
    # (برای پایداریِ آموزش، هرازگاهی انجام می‌شود).
    def update_target_network(self):
        """Copy weights from main network to target network."""
        self.target_network.load_state_dict(self.q_network.state_dict())

    # [فارسی] select_action: انتخابِ جهت با سیاستِ epsilon-greedy و ماسکِ اکشن‌های مجاز.
    # اکتشاف = جهتِ مجازِ تصادفی؛ بهره‌برداری = بیشترین Q بینِ جهت‌های مجاز (نامجازها -inf).
    ...
    # [فارسی] remember: یک تجربه را در بافر ذخیره می‌کند و مجموعِ پاداش را به‌روز می‌کند.
    def remember(self, state, action, reward, next_state, done):
        """Store experience in replay buffer."""
        self.memory.push(state, action, reward, next_state, done)
        self.total_reward += reward

    # [فارسی] train: یک گامِ یادگیری. Qِ فعلی و Qِ هدف را حساب می‌کند، خطای MSE می‌گیرد،
    # گرادیان می‌زند (با کلیپِ گرادیان)، epsilon را کم می‌کند و هرازگاهی شبکهٔ هدف را
    # هم‌گام می‌کند. (تفاوت با Proposed: اینجا MSE است، آنجا Huber.)
    def train(self):
        """
        ...
        # [فارسی] Qِ فعلی برای اکشن‌هایی که واقعاً انتخاب شده بودند.
        current_q_values = self.q_network(states).gather(1, actions.unsqueeze(1))

        # Compute next Q-values using target network
        # [فارسی] هدف = پاداش + γ × بهترین Qِ حالتِ بعدی (اگر اپیزود تمام نشده باشد).
        with torch.no_grad():
            next_q_values = self.target_network(next_states).max(1)[0]
            target_q_values = rewards + (self.gamma * next_q_values * ~dones)

        # Compute loss (Mean Squared Error)
        # [فارسی] خطا = میانگینِ مربعاتِ اختلافِ Qِ فعلی و هدف.
    ...
    # [فارسی] save_model / load_model: ذخیره و بازیابیِ وزن‌ها، بهینه‌ساز، epsilon و
    # شمارهٔ گامِ آموزش در فایلِ .pth (برای ادامهٔ آموزش یا حالتِ ارزیابی).
...
# [فارسی] DeepNR_Server: سرورِ ZMQ که با روترهای gem5 حرف می‌زند. روتر «حالت» را
# به‌صورتِ JSON می‌فرستد و سرور «اکشن» (جهت) برمی‌گرداند. آرگومان‌های اضافه (داشبورد،
# مانیتورِ حالت) اختیاری‌اند و برای فهمِ منطقِ اصلی لازم نیستند.
class DeepNR_Server:
    """
    ...
    # [فارسی] constructor: سوکتِ ZMQ از نوعِ REP را روی پورتِ 5555 می‌سازد و گزینه‌ها را ذخیره می‌کند.
    def __init__(
        self,
        agent,
        port=5555,
    ...
    # [فارسی] *** run: حلقهٔ اصلیِ سرور (مهم‌ترین متد) ***
    # تکرارِ بی‌نهایت: ۱) حالت را از gem5 می‌گیرد، ۲) تجربهٔ گامِ قبل را (حالا که پاداشش
    # رسیده) در بافر ذخیره می‌کند، ۳) با select_action یک جهت انتخاب می‌کند، ۴) اکشن را
    # برمی‌گرداند، ۵) هر چند بسته یک‌بار train() را صدا می‌زند و مدل را ذخیره می‌کند.
    # (بخشِ زیادی از طولِ این متد، لاگ‌گیری و داشبوردِ اختیاری است.)
    def run(self):
...
# [فارسی] main: آرگومان‌های خطِ فرمان را می‌خواند، عامل را با ابرپارامترها می‌سازد،
# در صورتِ وجود مدلِ ذخیره‌شده آن را بارگذاری می‌کند، سرور را می‌سازد و run() را صدا می‌زند.
def main():
